...
| [agent-dispatch.md](agent-dispatch.md) | Freezing the session on a foreground subagent, polling for one, prompts that dictate HOW, slices that span two owners so the agent hits a deny and hacks around it, and blaming a half-done slice on the agent that ran it |
