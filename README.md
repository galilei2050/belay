...
| [claude-md-hook](plugins/claude-md-hook) | Skill + PostToolUse | Keeps every CLAUDE.md on one skeleton — stamps the `Updated:` date on each edit and blocks on a missing Business/Technical decisions section, 200+ lines, a dead path, or a decision duplicated from a parent file; the same script audits a repo and flags files the code has left behind |
