...
the backstop for when it is ignored. Both read the repository — `git rev-list --count`,
`gh pr view` — rather than the output of the command that just ran, so a failed push is not
...
- When `gh` cannot answer (missing, unauthenticated, offline). "No PR found" and "I could not
  look" are different answers, and only the first is worth a nudge.
